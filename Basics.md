...
UFDs. A ring has infinitely many ideals. Each ideal has only one distinct 
factorization into ideals, even in rings where numbers have multiple distinct 
factorizations.

An ideal is generated by one or two numbers in the ring. One number generates a 
principal ideal. Two numbers may generate what I call, for lack of a better 
term, a secondary ideal.

Given a ring $R$ such as $\mathbb Z$ and a number $a \in R$, the **principal 
ideal** $\langle a \rangle$ is simply the set of all numbers of the form $ax$, 
where $x \in R$ also. Then $a$ is said **to generate** the ideal $\langle a 
\rangle$.

For example, in $\mathbb Z$, the principal ideal $\langle 3 \rangle$ simply 
consists of 0 and all negative and positive multiples of 3. The number 3 
generates the ideal $\langle 3 \rangle$.

An ideal generated by a number in a ring is exactly equal to an ideal generated 
by that same number multiplied by any unit in the ring. Algebraically, $\langle 
a \rangle = \langle ua \rangle$, where $a \in R$ and $u$ is any unit in $R$. For 
example, $\langle 3 \rangle = \langle -3 \rangle$.
...
The ideal $\langle a, b \rangle$, consisting of all numbers of the form $ax + 
by$, where $a$, $b$, $x$ and $y$ are all in $R$, is a **secondary ideal** *if* 
the ideal can't be generated by any one single number in $R$.
...
$\langle 38 \rangle$ is not a prime ideal. Indeed $\langle 38 \rangle = \langle 
2 \rangle \langle 19 \rangle$.
...
#### Examples of multiple distinct factorizations of numbers
...
Those are two distinct factorizations into irreducibles. Neither 2 nor 3 is 
divisible by either of $1 \pm \sqrt{-5}$, nor the other way around. For example, 
$$\frac{1 + \sqrt{-5}}{2}$$ has minimal polynomial $2x^2 - 2x + 3$, so it's not 
an algebraic integer. Likewise, we see that $$\frac{2}{1 + \sqrt{-5}}$$ has 
minimal polynomial $3x^2 - 2x + 2$, so that's not an algebraic integer either. 
The reader may verify the other divisions don't produce algebraic integers 
either.
...
#### Factorization into ideals

I reiterate that factorization into ideals is unique even when factorization 
into numbers is not. But in rings like $\mathbb Z[\sqrt{-5}]$, many prime ideals 
are secondary ideals, meaning that they are each generated by two distinct 
coprime numbers.

For example, in $\mathbb Z$, the ideals $\langle 2 \rangle$ and $\langle 2 
...
\of{19} + (\root 3 \of{19})^2$ has norm 198, with minimal polynomial $x^3 - 
9x^2 - 87x - 198$.
